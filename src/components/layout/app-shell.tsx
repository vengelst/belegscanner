...
export function AppShell({ children, userName, userRole }: AppShellProps) {
  const navItems = userRole === "ADMIN" ? [...commonNav, ...adminNav] : commonNav;
          ...
          <nav className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  "rounded-2xl border border-border/80 bg-background/80 px-4 py-3 text-center text-sm font-medium",
